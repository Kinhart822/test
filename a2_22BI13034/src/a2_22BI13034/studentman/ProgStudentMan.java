...
import a2_22BI13034.kengine.Doc;
import a2_22BI13034.kengine.DocCnt;
import a2_22BI13034.kengine.Engine;
import a2_22BI13034.kengine.Query;
...
public class ProgStudentMan {
  ...
  public Query search(String[] words) throws NotPossibleException{
	    if (words == null || words.length == 0) {
	      throw new NotPossibleException("Invalid search keywords");
	  }

	  Query query = null;
	  for (String word : words) {
	      if (query == null) {
	          query = engine.queryFirst(word);
	      } else {
	          Query newQuery = engine.queryMore(word);
	          if (newQuery.size() == 0) {
	              throw new NotPossibleException("Failed to execute query using provided keywords");
	          }
	          query = newQuery;
	      }
	  }
